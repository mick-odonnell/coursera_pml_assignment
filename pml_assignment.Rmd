...
The above result is not great;

>>>   .metric  .estimator .estimate
>>>   <chr>    <chr>          <dbl>
>>> 1 accuracy multiclass     0.391
>>> 2 kap      multiclass     0.222

It might be worthwhile running a tuning process to try to improve the model's predictive accuracy, and to use cross-validation also. 
...
glmnet_model <- train(x = exercise_training_glm_prepped %>% select(-classe),
                   y = exercise_training_glm_prepped$classe,
                   method = "glmnet",
                   family = "multinomial",
                   trControl = ctrl)
```
